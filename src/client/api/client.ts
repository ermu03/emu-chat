...
export class EmuChatApiClient {
  ...
  getConversation(conversationId: string): Promise<ConversationDetailResponse> {
    return this.request<ConversationDetailResponse>(`/api/v1/conversations/${conversationId}`);
  }
  ...
  getDraft(conversationId: string): Promise<DraftResponse> {
    return this.request<DraftResponse>(`/api/v1/conversations/${conversationId}/draft`);
  }

  putDraft(conversationId: string, data: PutDraftRequest): Promise<DraftResponse> {
    return this.request<DraftResponse>(`/api/v1/conversations/${conversationId}/draft`, {
      method: 'PUT',
      body: JSON.stringify(data)
    });
  ...
  patchQueueItem(queueItemId: string, data: PatchQueueItemRequest): Promise<QueueItemResponse> {
    return this.request<QueueItemResponse>(`/api/v1/queue-items/${queueItemId}`, {
      method: 'PATCH',
      body: JSON.stringify(data)
    });
  }

  cancelQueueItem(queueItemId: string, data: CancelQueueItemRequest): Promise<QueueItemResponse> {
    return this.request<QueueItemResponse>(`/api/v1/queue-items/${queueItemId}/cancel`, {
      method: 'POST',
      body: JSON.stringify(data)
    });
  }
  ...
  discardRecovery(queueItemId: string): Promise<{ discarded: boolean }> {
  ...
  stopRun(localRunId: string): Promise<{ stopping: boolean }> {
    return this.request<{ stopping: boolean }>(`/api/v1/runs/${localRunId}/stop`, {
      method: 'POST',
      body: JSON.stringify({})
    });
  }

  submitApproval(localRunId: string, data: ApprovalRequest): Promise<{ accepted: boolean }> {
    return this.request<{ accepted: boolean }>(`/api/v1/runs/${localRunId}/approval`, {
      method: 'POST',
      body: JSON.stringify(data)
    });
  }
  ...
  reconcileRun(localRunId: string): Promise<ReconcileResponse> {
    return this.request<ReconcileResponse>(`/api/v1/runs/${localRunId}/reconcile`, {
      method: 'POST',
      body: JSON.stringify({})
    });
  }
