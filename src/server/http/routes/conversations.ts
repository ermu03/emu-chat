...
> = async (fastify: FastifyInstance, options: ConversationRoutesOptions) => {
  ...
  fastify.post("/conversations", async (req, reply) => {
    const parseResult = CreateConversationRequestSchema.safeParse(
      req.body ?? {},
    );
    if (!parseResult.success) {
      throw new ValidationError(
  ...
  fastify.patch(
    "/conversations/:conversationId/hermes-metadata",
    async (req, reply) => {
      const { conversationId } = req.params as { conversationId: string };
      ...
      const res = await conversationService.updateHermesMetadata(
        conversationId,
        parseResult.data,
      );
      return reply.status(200).send(res);
  ...
  fastify.patch(
    "/conversations/:conversationId/local-metadata",
    async (req, reply) => {
      const { conversationId } = req.params as { conversationId: string };
      ...
      const res = await conversationService.updateLocalMetadata(
        conversationId,
        parseResult.data,
      );
      return reply.status(200).send(res);
  ...
  fastify.post("/conversations/:conversationId/fork", async (req, reply) => {
    ...
    const res = await conversationService.forkConversation(
      conversationId,
      parseResult.data,
    );
    return reply.status(201).send(res);
  ...
  fastify.post("/conversations/:conversationId/reset", async (req, reply) => {
    const { conversationId } = req.params as { conversationId: string };
    const parseResult = ResetConversationRequestSchema.safeParse(
      req.body ?? {},
    );
    if (!parseResult.success) {
      throw new ValidationError(
    ...
    const res = await conversationService.resetConversation(
      conversationId,
      parseResult.data,
    );
    return reply.status(201).send(res);
  ...
  fastify.post("/conversations/:conversationId/delete", async (req, reply) => {
    ...
    const res = await conversationService.deleteConversation(
      conversationId,
      parseResult.data,
    );
    return reply.status(200).send(res);
