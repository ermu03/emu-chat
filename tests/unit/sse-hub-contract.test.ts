...
describe("SSEHub browser run stream contract", () => {
  ...
  it("cleans up subscribers and discards a run replay window", () => {
    const hub = new SSEHub();
    hub.publishRunEvent("lr_cleanup", 1, "message.delta", { delta: "old" });
    const subscriber = makeReply();
    hub.subscribe("lr_cleanup", subscriber.reply);

    hub.cleanup("lr_cleanup");

    expect(subscriber.raw.end).toHaveBeenCalledOnce();
    const reconnect = makeReply();
    hub.subscribe("lr_cleanup", reconnect.reply, 0);
    expect(reconnect.writes).toEqual([
      'event: stream.ready\ndata: {"local_run_id":"lr_cleanup","earliest_seq":1,"latest_seq":0}\n\n',
    ]);
    hub.close();
  });

  it("releases terminal replay state after its configured retention period", () => {
    vi.useFakeTimers();
    const hub = new SSEHub();
    const subscriber = makeReply();
    hub.publishRunEvent("lr_terminal", 1, "run.reconciled", {});
    hub.subscribe("lr_terminal", subscriber.reply);

    hub.scheduleCleanup("lr_terminal", 60_000);
    vi.advanceTimersByTime(59_999);
    expect(subscriber.raw.end).not.toHaveBeenCalled();
  ...
  it("ends active browser responses on shutdown and does not recreate buffers afterward", () => {
    const hub = new SSEHub();
    const subscriber = makeReply();
    hub.subscribe("lr_close", subscriber.reply);

    hub.close();
    hub.publishRunEvent("lr_close", 1, "message.delta", { delta: "ignored" });
    const reconnect = makeReply();
    hub.subscribe("lr_close", reconnect.reply, 0);

    expect(subscriber.raw.end).toHaveBeenCalledOnce();
    expect(reconnect.raw.end).toHaveBeenCalledOnce();
    expect(reconnect.writes).toEqual([]);
  });
});
