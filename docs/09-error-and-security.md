...
6. **`review_required`**: 枚举保留值；当前服务端错误类没有返回此动作
7. **`reconnect`**: 建议重新连接
...
### 数据目录权限控制
启动入口会将运行时数据目录设为 `0o700`，数据库连接初始化会尝试将 SQLite 主文件设为 `0o600`。这段代码没有逐一设置 WAL、SHM 等侧文件的权限。
...
### 数据生命周期管理
- **载荷清除**: 队列项成功完成或取消时，`payload_text` 设为 `NULL`；失败后的恢复载荷可由用户手动丢弃，或在 7 天恢复期限到达后由后台分批清空。期限一到，API 即停止提供和复制正文。
- **控制记录期限**: 完成或取消的队列项保留 7 天；已过期或主动丢弃正文的 `paused`、`rejected` 项从最后更新时间起保留 30 天。关联的终态 Run 随队列项级联删除；待人工处理或活跃的 Run 保留。清空或删除会产生可复用页，不保证 SQLite 文件立即缩小。

### 请求安全性
- **API Key 隔离**: Hermes API Key 仅在服务端环境注入，前端绝不接触。
- **敏感头清理**: 代理转发时显式删除 `X-Hermes-Session-Key` 等潜在敏感头。
- **非 SSE 响应大小**: `readBoundedText` 在上游声明的 `Content-Length` 超过 4 MiB 时提前拒绝；否则读取完整正文后检查大小，目前没有逐块限制读取量。
- **SSE 防御**: HermesClient 每次读取 chunk 后、分行前检查 SSE 字符串缓冲区是否超过 256 KiB；当前不是整帧累计大小限制。

---

## 主要限制常量

以下值来自 [LIMITS](../src/shared/limits.ts)；草稿防抖和消息滚动阈值分别定义在客户端组件中，不属于该对象。

| 范围 | 当前限制 |
| --- | --- |
| 输入 | 消息正文 64 KiB UTF-8；JSON 请求体 128 KiB；标题 200 字符；单个标签 40 字符、最多 20 个标签 |
| 队列与并发 | 发送接口对每会话非 `done`/`cancelled` 队列项设置 100 条上限；全局和每会话各最多 1 个活跃 Run |
| 分页 | 会话列表默认 50、最多 100；消息列表默认 100、最多 200 |
| 租约与幂等 | 租约 TTL 15 秒、心跳间隔 5 秒；Hermes 必须声明至少 24 小时的幂等键保留期，派发时记录相应的准入截止时间 |
| 数据保留 | 失败恢复正文 7 天；`done`/`cancelled` 控制记录 7 天；已清除正文的 `paused`/`rejected` 控制记录 30 天 |
| SSE | 每 Run 最多 5 个连接、全局最多 20 个；事件环最多 512 条或 1 MiB；终态回放保留 60 秒；上游接收缓冲区分行前检查 256 KiB 上限 |
