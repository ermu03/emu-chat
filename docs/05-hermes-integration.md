...
- **request<T> 管道**:
  ...
  4. 使用 `readBoundedText` 控制最多接收 4MiB 的响应，避免内存溢出。
  5. 经过 `throwForStatus` 的状态码映射，转换为特定业务异常（如 `NotFoundError`, `UnauthorizedError`）。
- **stream SSE管道**:
  - **握手超时**: 限制 10s 内必须完成连接。
  - **readSseChunk竞速**: 利用 `Promise.race` 检查网络活跃度（Liveness 保活上限 30s）。
  - **行解析状态机**: 按行增量解析协议。
  - **单帧限制**: 确保单个 SSE 数据帧不超过 256KiB。
...
- **强校验 (Zod Schema)**: 所有适配器的响应都必须经过 Zod 验证过滤；对于不符约定的格式偏差，统一向外抛出 `HermesProtocolError` 异常。
...
- **评估结果**: 最终返回四种状态之一，包括 `healthy`, `degraded`, `incompatible`, 或 `unreachable`，提供给调用方以便进行降级策略。
...
stateDiagram-v2
    ...
    CheckRuntime --> Degraded : 部分非致命缺失
    CheckRuntime --> Incompatible : 关键能力缺失
    CheckRuntime --> Unreachable : 网络不通/无响应
```
