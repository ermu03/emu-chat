...
- **SSEHub（事件广播中枢）**: 接收 Hermes 返回的实时流 (Server-Sent Events) 并管理连接的生命周期，向连接的前端广播状态变更和 Token 流。

### 持久层 (SQLite 仓储)
由 5 个专注单一领域的仓储构成：
- `ConversationRepo`: 本地会话的元数据缓存。
- `DraftRepo`: 尚未发送的草稿内容。
- `PreferencesRepo`: 存储用户的个性化配置。
- `QueueRepo`: 待发送或排队中的请求序列。
- `RunRepo` & `LeaseRepo`: 本地任务运行和并发锁的信息。

### Hermes 集成层
与上游交互的核心：
- `HermesClient`: 基于原生 Fetch 和 eventsource-parser 封装的 HTTP/SSE 客户端。
...
flowchart TD
    ...
    Main --> DraftComposer["DraftComposer<br/>(富文本编辑输入，自动存草稿)"]
...
通过前后端共享一个 npm 命名空间 (`shared/`)，实现了**类型零漂移（Type Zero-drift）**。主要作用包括：
- **Zod Schema**: 约束所有 API 请求的 Request Body 和 Response Payload，使得在客户端编译时即可验证前后端类型兼容性。
- **共享枚举**: API 的错误码定义、状态机的枚举值。
- **ID 生成**: 统一提供 `genReqId` 逻辑和特定资源 ID 的前缀约束。
- **限制常量**: 约定最大并发数、内容限制、字符串截断长度等硬编码限制 `LIMITS`，确保双端表现一致。
...
3. **优雅停机 (Graceful Shutdown)**:
   - `preClose` 钩子：先停止后台循环 `stopCoordinator()` 并切断所有的持久化 SSE 流连接 `closeSseHub()`。
   - `onClose` 钩子：最终释放数据库连接 `db.close()`。

## 7. 请求 ID 链路追踪

为了易于诊断异常，整个系统严格贯穿了请求追踪机制：
- 使用 Fastify 提供的 `genReqId` 定制逻辑：若前端没传 `x-request-id` 头，则自动生成前缀为 `rq_` 的 Request ID。
