...
现有 docs/ 说明系统当前如何工作；值得跨会话跟进的未完成问题和改进方案记录在 .agents/notes/proposed/，不另设待办清单。复杂需求的规格可按需放在 .agents/specs/，需要长期复用的研究资料可按需放在 .agents/research/。笔记解释问题、候选方案及重要取舍，不复述接口字段或代码流程；需要细节时链接到现有文档或源码。

## 何时写

行为、架构、跨模块约定、持久化或网络格式、测试策略、开发流程发生非平凡变化，或者未来维护者只看代码和测试仍无法推断原因时，写或更新一篇笔记。未完成但值得跨会话跟进的问题，也在 proposed 记录症状、已知事实、候选方案和验收条件；相互依赖的事项合并成一篇。纯格式化、样式调整、常规 CRUD 和局部显而易见的修复不写。
...
| 状态 | 用途 | 处理方式 |
| --- | --- | --- |
| proposed | 尚未落地的问题或方案，也是待处理事项入口 | 写清已知事实、候选方案、真实备选、验收条件和风险；待验证的判断注明不确定性 |
...
1. 有值得记录的新问题或方案时先写 proposed；实现完成且决定仍有参考价值时，在同一次代码变更中移入 implemented。将 Status 改为 implemented，把 Proposal 改写成描述已落地事实的 Decision，并把验收结果、收益和代价写进 Consequences。一次性问题解决后若没有保留价值，可删除提案。
...
链接笔记时使用相对 Markdown 路径。备选方案只记实际比较过的选项：先说明它最强的理由，再解释为何没选；不要为了凑数量编造方案。多阶段工作可在 Proposal 下附简短实施步骤，Acceptance criteria 仍写可观察结果。已实施笔记的 Consequences 同时写收益和代价。
