...
引入这套笔记时，[docs/](../../../../docs/) 描述系统的当前结构，根目录 `ISSUES.md` 保存待办，但重要取舍、被放弃的方案和后来应如何重新审视决定没有统一位置。独立开发时，新一轮开发或 AI 会话容易只看到代码现状，重复讨论旧方案。待办后来也已[并入 proposed 笔记](2026-09-23-track-pending-work-in-proposed-notes.md)。
...
- **继续只用当时的 docs/ 和 ISSUES.md**：文件少，系统和待办都能找到；但缺少对提案、已实施决定、否决方案和历史决定的明确区分，也难以定位某项约束的由来。
